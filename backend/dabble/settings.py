...
# Upsun publishes the environment's domains only at runtime, and every preview
# branch gets its own, so take the hosts from the injected routes when present.
_routes = env("PLATFORM_ROUTES")
if _routes:
    ALLOWED_HOSTS = sorted(
        {
            host
            for host in (
                urlparse(url).hostname for url in json.loads(base64.b64decode(_routes))
            )
            if host
        }
    )
...
ROOT_URLCONF = "dabble.urls"
WSGI_APPLICATION = "dabble.wsgi.application"
ASGI_APPLICATION = "dabble.asgi.application"
...
DEFAULT_FROM_EMAIL = env("DEFAULT_FROM_EMAIL", "hello@dabble.app")
EMAIL_BACKEND = (
    "django.core.mail.backends.console.EmailBackend"
    if DEBUG
    else "django.core.mail.backends.smtp.EmailBackend"
)
# Upsun runs an SMTP relay per environment and injects its host; no auth, port 25.
EMAIL_HOST = env("EMAIL_HOST") or env("PLATFORM_SMTP_HOST")
EMAIL_PORT = env_int("EMAIL_PORT", 25)
EMAIL_HOST_USER = env("EMAIL_HOST_USER")
EMAIL_HOST_PASSWORD = env("EMAIL_HOST_PASSWORD")
EMAIL_USE_TLS = env_bool("EMAIL_USE_TLS", False)

APPLE_BUNDLE_ID = env("APPLE_BUNDLE_ID", "app.dabble.ios")
...
MEDIA_BUCKET = env("MEDIA_BUCKET", "dabble-media")
...
if not DEBUG:
    ...
    # CI is set during a build hook, where the mail relay isn't attached yet.
    if not EMAIL_HOST and not env_bool("CI", False):
        raise ImproperlyConfigured("EMAIL_HOST is required to send sign-in codes.")
