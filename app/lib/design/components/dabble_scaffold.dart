...
import 'dabble_header.dart';

/// Every screen in Dabble is built on this.
///
...
/// Material AppBar.
class DabbleScaffold extends StatelessWidget {
  const DabbleScaffold({
    super.key,
    required this.child,
    this.title,
    this.trailing,
      ...
      slivers: [
        if (title != null)
          CupertinoSliverNavigationBar(
            largeTitle: Text(title!, style: DabbleText.title1.on(c.ink)),
...
class DabblePage extends StatelessWidget {
  const DabblePage({
    super.key,
    required this.child,
    this.title,
